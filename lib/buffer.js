...
Teleinfo.prototype.parse = function(packet) {
  ...
  if(ptec.match(/\.\.$/)) ptec = 'HC' + ptec.slice(0, 2)
  ...
  if (now.getTime() > buffer.timestamp + self._init.delay) {
    ...
    if (packet.OPTARIF.match(/BBR/) && packet.DEMAIN !== '----' &&  ((now.getHours() > 20 && buffer.tomorrow.date < now.setHours(0,0,0,0)) || !buffer.tomorrow.date)) {
